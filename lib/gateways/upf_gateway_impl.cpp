...
bool upf_gateway_impl::is_initialized()
{
  return sock_fd != -1;
}

void upf_gateway_impl::handle_pdu(byte_buffer pdu, const sockaddr_storage& dest_addr)
{
  auto fn = [this, p = std::move(pdu), dest_addr]() mutable { handle_dn_pdu_impl(std::move(p), dest_addr); };
  if (not io_tx_executor.execute(std::move(fn))) {
    logger.info("Dropped PDU to DN, queue is full.");
  }
}

void upf_gateway_impl::handle_upf_pdu(byte_buffer pdu, const sockaddr_storage& dest_addr)
{
  auto fn = [this, p = std::move(pdu), dest_addr]() mutable { handle_pdu_impl(std::move(p), dest_addr); };
  if (not io_tx_executor.execute(std::move(fn))) {
    logger.info("Dropped PDU to vUPF, queue is full.");
  }
}

void upf_gateway_impl::handle_pdu_impl(byte_buffer pdu, const sockaddr_storage& dest_addr)
{
...
void upf_gateway_impl::handle_dn_pdu_impl(byte_buffer pdu, const sockaddr_storage& dest_addr)
{
  logger.debug("Sending DN PDU of {} bytes", pdu.length());

  if (fd == -1) {
    logger.error("Fd not initialized");
    return;
  }

  if (pdu.length() > network_gateway_udp_max_len) {
    logger.error("DN PDU of {} bytes exceeds maximum length of {} bytes", pdu.length(), network_gateway_udp_max_len);
    return;
  }
  ...
  int bytes_sent =
      sendto(fd, pdu_span.data(), pdu_span.size_bytes(), 0, (sockaddr*)&dest_addr, sizeof(sockaddr_storage));
  if (bytes_sent == -1) {
    std::string local_addr_str;
    std::string dest_addr_str;
    sockaddr_to_ip_str((sockaddr*)&dest_addr, dest_addr_str, logger);
    sockaddr_to_ip_str((sockaddr*)&local_addr, local_addr_str, logger);
    logger.error("Couldn't send {} B of data on socket: local_ip={} dest_ip={} error=\"{}\"",
                 pdu_span.size_bytes(),
                 local_addr_str,
                 dest_addr_str,
                 strerror(errno));
...
bool upf_gateway_impl::create_and_bind()
{
  ...
  std::string local_addr_str;
  sockaddr_to_ip_str((sockaddr*)&local_addr, local_addr_str, logger);
  logger.debug("local_ip={}",local_addr_str);
...
void upf_gateway_impl::receive()
{
  ...
  for (int i = 0; i < rx_msgs; ++i) {
    ...
    if (pdu.append(payload)) {
      ...
      logger.debug("UPF GW Received PDU = {}", pdu);
      byte_buffer_reader pdu_reader = pdu;
      uint8_t qfi = *pdu_reader;
      logger.debug("UPF GW Received QFI = {}", qfi);
      pdu.trim_head(1);
      logger.debug("UPF GW Received PDU = {}", pdu);
      data_notifier.on_new_pdu(std::move(pdu), uint_to_qos_flow_id(qfi));
    } else {
...
int upf_gateway_impl::get_socket_fd()
{
  return sock_fd;
}

optional<uint16_t> upf_gateway_impl::get_bind_port()
{
  if (not is_initialized()) {
...
bool upf_gateway_impl::get_bind_address(std::string& ip_address)
{
...
bool upf_gateway_impl::set_sockopts()
{
...
bool upf_gateway_impl::set_non_blocking()
{
...
bool upf_gateway_impl::set_receive_timeout(unsigned rx_timeout_sec)
{
  struct timeval tv;
...
bool upf_gateway_impl::set_reuse_addr()
{
  int one = 1;
...
bool upf_gateway_impl::close_socket()
{
  if (not is_initialized()) {
    return true;
  }
