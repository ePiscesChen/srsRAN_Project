...
class upf_gateway_impl final : public udp_network_gateway
{
public:
  explicit upf_gateway_impl(udp_network_gateway_config                   config_,
                            network_gateway_data_notifier_with_src_addr& data_notifier_,
                            task_executor&                               io_executor_);
  ~upf_gateway_impl() override { close_socket(); }
...
private:
  ...
  void handle_pdu_impl(byte_buffer pdu, const sockaddr_storage& dest_addr);
  void handle_dn_pdu_impl(byte_buffer pdu, const sockaddr_storage& dest_addr);
  ...
  srs_cu_up::sdap_tx_sdu_handler*              sdap_handler = nullptr;
  ...
  std::array<uint8_t, 200> tx_mem;
  uint32_t network_gateway_udp_max_len = 9100;
};
